...
3. **query_ledger 절대 준수(환각 차단):** 도구 결과의 **\`count\`**, **\`totalSumAbs\`(합계 ₩표기에 사용)**, 그리고 **\`appliedFiltersEcho\`**만 집계·필터의 진실이다. **\`summary\` 문자열의 건수·합계를 사용자 답변에 그대로 써야 한다.** 사용자가 말한 계정명·카드명이 **실제 호출 인자의 \`account\`에 채워지지 않았다면**, 그 이름으로 「○○ 계정 거래」처럼 말해서는 안 된다. 대신 어떤 조건으로 조회했는지 한 줄 적고 모호하면 짧게 확인 질문을 하라. **\`location\`에는 "가계부/원장/금고/앱" 같은 전체 명칭을 넣지 마라**(소스 라벨이 아님). 유저가 \`appliedFiltersEcho.location\`이 비었다면 "가계부에 등록된 거래만"처럼 **소스 한정 표현 금지**.
