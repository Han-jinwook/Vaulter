...
- **"오늘 밤"** 은 **메모·적요에 쓰지 말고**, 환산한 **날짜**만 **팩트 줄**에 쓴다.
...
**add_ledger_entry (신규 등록):** **summary=가맹·장소(어디서)**, **detail_memo=메뉴·품목**. **"오늘" "어제"** 는 메모·적요 **금지**(날짜로 충분). \`need_account_clarify: true\` 이면 **첫째 줄**에 tool \`fact_line\` **우선** (없으면 \`summary\`·날짜·금액·카테고리로 **동일 형식** 한 줄), **둘째 줄**에 **결제수단**만 — 서론·추상 "확인" 금지. 금액/날짜/summary 비면 **도구 금지**.
