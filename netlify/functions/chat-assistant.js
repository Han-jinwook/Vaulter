...
- **\`summary\` (적요·필수):** **무엇에 쓴 돈인지** 키워드만(떡볶이, 돼지국밥, 택시). **서술어·~함·먹음** 금지. **"어제 밤" "오늘 점심"** 같은 **시간·상황**은 적요에 **넣지 말고** \`detail_memo\`에 넣는다. "지기"·"AI" 금지.
- **\`detail_memo\` (시간·맥락, 강력 권장):** 유저가 **어제/어제 밤/오늘/점심/저녁/출근길** 등 **언제·어떤 상황**을 말했으면, 그걸 **원장 메모 열**에 쓰기 위해 **여기**에 넣는다(예: "어제 밤", "오늘 점심"). 품목 키워드만 summary에, 시간 맥락은 **반드시** detail_memo로 **분리**해도 좋다. **추가로** 쓸 말이 전혀 없을 때만 생략.
...
const TOOLS = [
  ...
  },
  {
    ...
    function: {
      ...
      description:
        '필수: summary(품목 키워드)·정확한 금액·날짜(YYYY-MM-DD)·category(Enum). "어제 밤" 등 시간·맥락은 **detail_memo**에(메모 열). summary는 **무엇에 쓴 돈인지**만. 금액/날짜/summary 없으면 도구 금지. category는 애매할 때만 역질문.',
      parameters: {
        type: 'object',
        properties: {
          type: {
          ...
          summary: {
            type: 'string',
            description:
              '**적요(원장 제목)**. **품목/가맹** 키워드만(떡볶이, 돼지국밥). **"어제 밤" "점심"** 은 **여기 쓰지 말고** detail_memo로. 서술어 금지.',
          },
          detail_memo: {
            type: 'string',
            description:
              '**메모 열(시간·맥락)**. 유저가 "어제밤" "어제 밤" "오늘 점심" 등 **언제**를 말하면 **필수에 가깝게** 채운다(예: "어제 밤"). summary(품목)와 **중복하지 말고** 시간·상황만. 없으면 생략.',
          },
          account: {
            type: 'string',
...
**add_ledger_entry (신규 등록):** [필수] **summary**(품목만), 금액, 날짜, category(Enum). **어제 밤·오늘 점심** 등 **시간 맥락 = detail_memo**(메모 열). tool 결과에 \`need_account_clarify: true\` 있으면 **답장 끝에** 결제수단 **질문 필수**. 금액/날짜/summary 비면 **도구 금지**.
